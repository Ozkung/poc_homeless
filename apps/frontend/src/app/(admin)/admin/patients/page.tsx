...
export default function AdminPatientsPage() {
  ...
  async function handleDelete(patientId: string) {
    if (!confirm('ยืนยันลบผู้ป่วยนี้?')) return;
    ...
    },
    {
      ...
      render: (_, r) => (
        <div style={{ display: 'flex', gap: 4 }} onClick={(e) => e.stopPropagation()}>
          ...
          <Button size="small" type="link" danger onClick={() => handleDelete(r.id)}>ลบ</Button>
        </div>
      ),
    },
  ];
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
        <div>
          <div style={{ fontSize: 10, color: '#7c3aed', letterSpacing: 3, textTransform: 'uppercase', marginBottom: 4 }}>Patients</div>
          <Title level={2} style={{ margin: 0, fontWeight: 800, letterSpacing: -1 }}>รายชื่อผู้ป่วย</Title>
        </div>
        <Link href="/admin/patients/new"><Button type="primary">+ เพิ่มผู้ป่วย</Button></Link>
      </div>
