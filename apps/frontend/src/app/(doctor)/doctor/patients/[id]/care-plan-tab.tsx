...
export default function DoctorCarePlanTab({ patientId }: { patientId: string }) {
    ...
    },
    {
      ...
      render: (_: any, row: Assessment) => (
        <div style={{ display: 'flex', gap: 6 }}>
          <Button size="small" icon={<Eye size={12} />} onClick={() => setViewItem(row)}>View</Button>
          <Button
            size="small"
            type="primary"
            icon={<Pencil size={12} />}
            onClick={() => router.push(`/doctor/patients/${patientId}/care-plan-assessment?id=${row.id}`)}
          >
            Update
          </Button>
        </div>
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        ...
        <Button
          ...
          type="primary"
          icon={<Plus size={13} />}
          onClick={() => router.push(`/doctor/patients/${patientId}/care-plan-assessment`)}
        >
          + เพิ่มแบบประเมิน
        </Button>
      </div>
        ...
        footer={
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
            ...
            <Button
              ...
              icon={<Pencil size={13} />}
              onClick={() => {
                setPendingNav(`/doctor/patients/${patientId}/care-plan-assessment?id=${viewItem?.id}`);
                setViewItem(null);
              }}
            >
              แก้ไข
