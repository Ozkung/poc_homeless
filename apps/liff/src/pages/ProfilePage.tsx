...
export default function ProfilePage() {
  ...
  const { lineProfile, systemProfile, zones, setSystemProfile, setZones, updateSystemProfile } = useProfileStore();

  const [form, setForm] = useState({
    displayName: '', email: '', phone: '', birthDate: '', gender: '', preferredZoneId: '',
  });
  ...
  const [loading, setLoading] = useState(!systemProfile); // skip load if store already has data
  const [notRegistered, setNotRegistered] = useState(false);

  // Populate form when systemProfile arrives (from store or fresh fetch)
  useEffect(() => {
    if (!systemProfile) return;
    setOriginalEmail(systemProfile.email);
    setForm({
      displayName: systemProfile.displayName ?? '',
      email: systemProfile.email,
      phone: systemProfile.phone ?? '',
      birthDate: toDateInput(systemProfile.birthDate),
      gender: systemProfile.gender ?? '',
      preferredZoneId: systemProfile.preferredZoneId ?? '',
    });
  }, [systemProfile?.id]);

  // Fetch only if store is empty (e.g. page opened directly via rich menu ?to=profile)
  useEffect(() => {
    if (systemProfile) { setLoading(false); return; }

    async function load() {
      try {
        const idToken = liff.getIDToken();
        if (!idToken) { navigate('/register', { replace: true }); return; }

        // LINE profile — put in store if not already there
        if (!lineProfile) {
          liff.getProfile().then((p) => {
            useProfileStore.getState().setLineProfile({
              userId: p.userId, displayName: p.displayName, pictureUrl: p.pictureUrl ?? undefined,
            });
          }).catch(() => {});
        }

        // Verify LINE ID → get fresh JWT → fetch system profile
        try {
          const { accessToken } = await api.verifyLiff(idToken);
          setToken(accessToken);
          const [me, zoneList] = await Promise.all([api.getMe(), api.getPublicZones()]);
          setSystemProfile(me);
          setZones(zoneList);
        } catch (e: any) {
  ...
  // Also ensure zones are loaded if they came from store with empty zones
  useEffect(() => {
    if (zones.length === 0 && systemProfile) {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      ...
      setCurrentPassword('');
      updateSystemProfile(updated); // sync store
  ...
  if (notRegistered) return (
    <div style={{ background: '#F8FAFC', minHeight: '100vh' }}>
      <div style={{ background: '#fff', borderBottom: '1px solid #F1F5F9', padding: '16px 16px 14px' }}>
        ...
        <h1 style={{ fontSize: 20, fontWeight: 700, color: '#0F172A', margin: 0 }}>โปรไฟล์</h1>
      </div>
      <div style={{ maxWidth: 480, margin: '0 auto', padding: 24, textAlign: 'center' }}>
        {lineProfile?.pictureUrl ? (
          <img src={lineProfile.pictureUrl} alt="LINE" style={{ width: 64, height: 64, borderRadius: '50%', objectFit: 'cover', border: '3px solid #fff', boxShadow: '0 0 0 3px #E0E7FF', marginBottom: 12 }} />
        ) : (
          <div style={{ width: 64, height: 64, borderRadius: '50%', background: '#E2E8F0', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, margin: '0 auto 12px' }}>
            {lineProfile?.displayName?.[0]?.toUpperCase() ?? '?'}
          </div>
        )}
        <div style={{ fontSize: 16, fontWeight: 700, color: '#0F172A', marginBottom: 4 }}>{lineProfile?.displayName ?? '—'}</div>
        <div style={{ fontSize: 12, color: '#94A3B8', marginBottom: 24 }}>LINE Account นี้ยังไม่ได้ลงทะเบียนในระบบ</div>
        <button onClick={() => navigate('/register')}
          style={{ width: '100%', maxWidth: 320, padding: 13, borderRadius: 12, border: 'none', background: ACCENT, color: '#fff', fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>
          สมัครสมาชิก →
        </button>
      </div>
    </div>
  ...
  const chipStyle: React.CSSProperties = {
    ...(ROLE_CHIP[systemProfile?.role ?? ''] ?? { background: '#F1F5F9', color: '#475569', borderColor: '#E2E8F0' }),
    display: 'inline-block', fontSize: 11, fontWeight: 700,
    padding: '3px 12px', borderRadius: 20, border: '1px solid',
  };
  const pic = lineProfile?.pictureUrl;
  const displayName = lineProfile?.displayName ?? systemProfile?.displayName ?? '?';
  ...
  return (
    <div style={{ background: '#F8FAFC', minHeight: '100vh', paddingBottom: 40 }}>
      ...
      <div style={{ background: '#fff', borderBottom: '1px solid #F1F5F9', padding: '24px 20px 20px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
        ...
        <div style={{ fontSize: 17, fontWeight: 700, color: '#0F172A' }}>{displayName}</div>
        ...
        {systemProfile && <span style={chipStyle}>{ROLE_LABEL[systemProfile.role] ?? systemProfile.role}</span>}
      </div>
