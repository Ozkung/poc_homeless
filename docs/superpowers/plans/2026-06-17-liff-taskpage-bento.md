# LIFF TaskPage Bento Redesign Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Redesign `TaskPage.tsx` into a bento layout — each event is a collapsible tile with a priority-colored header, patients inside are arranged in an adaptive 2-column grid.

**Architecture:** Single-file rewrite of `apps/liff/src/pages/TaskPage.tsx`. Data fetching and API calls are unchanged. New style constants replace the old Tailwind-only approach for the event/patient cards. Collapse state is tracked with `useState<Set<string>>`.

**Tech Stack:** React 19, React Router DOM v7, TypeScript, Tailwind CSS + inline styles, Vite
...
| File | Action |
|---|---|
| `apps/liff/src/pages/TaskPage.tsx` | Full rewrite — same data fetching, completely new render |

---

### Task 1: Rewrite TaskPage.tsx with bento layout
...
Replace the full contents of `apps/liff/src/pages/TaskPage.tsx` with:
...
git commit -m "feat(liff): redesign TaskPage with bento event tiles and adaptive patient grid"
```
...
After the task is complete, verify the following visually in the LIFF simulator or browser:

**Layout:**
- [ ] Event tiles have rounded corners and drop shadow
- [ ] CRITICAL event header is red gradient, URGENT is amber, NORMAL is blue
- [ ] Priority label badge appears in the header meta row
- [ ] Chevron ▼ rotates when collapsed

**Collapsible:**
- [ ] Tapping the event header hides/shows the patient grid
- [ ] Multiple events can be independently collapsed
- [ ] Events start expanded on page load

**Patient tiles:**
- [ ] CRITICAL events and single-patient events → all tiles span full width
- [ ] URGENT/NORMAL events with 2+ patients → half-width grid (2 columns)
- [ ] CRITICAL/URGENT priority dot appears top-right of tile (red/amber glow)
- [ ] DONE/NOT_FOUND patients → `opacity: 0.55`, no action buttons shown

**Action buttons:**
- [ ] Purple filled button appears when `formTemplate` is present
- [ ] "Check-in" button appears only for PENDING tasks
- [ ] "บันทึก" always appears for active tasks
- [ ] "แผนดูแล" appears when `patient.id` is present

**Edge cases:**
- [ ] Empty task list → shows "ไม่มีงานในพื้นที่ขณะนี้"
- [ ] Loading state → skeleton placeholders shown
- [ ] SOS button remains fixed at bottom while scrolling
